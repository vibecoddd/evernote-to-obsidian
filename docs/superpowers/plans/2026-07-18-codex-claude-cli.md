...
**Architecture:** 新增一个 argparse 适配层 `src/agent_cli.py`，只负责参数、配置覆盖、调用现有转换器/迁移器和输出结果；新增 `pyproject.toml` 以 console script 暴露该适配层。对现有转换器只做配置注入顺序的兼容性修复，保留旧脚本和 Web 入口。
...
- Modify: `src/evernote2obsidian.py` — 允许注入已合并的 `Config`，并修复旧 Click 入口的初始化顺序。
