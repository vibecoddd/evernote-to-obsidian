...
新增 `pyproject.toml`，项目名为 `evernote-to-obsidian`，console script 使用 `agent_cli:main` 指向 `src/agent_cli.py` 的 `main()`。现有 `src` 模块以顶层 Python 模块方式打包，避免为本次功能重写所有历史 import。
...
- 调用已有 `EvernoteToObsidianConverter` 或 `UnifiedMigrator`；
- 将内部统计转换为文本或 JSON；
- 将异常映射为稳定退出码。

适配层不复制 ENEX 转换逻辑。为保证配置在组件初始化前生效，`EvernoteToObsidianConverter` 增加接收已构造 `Config` 的能力，并修正现有入口中“先用默认配置初始化、再替换配置”的顺序问题。
