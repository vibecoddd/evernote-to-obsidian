...
description: Use when a user wants to migrate Evernote ENEX files or an Evernote account into an Obsidian vault, preview the migration, inspect migration JSON, or reset migration state with the evernote-to-obsidian CLI.
---
...
Treat reset as a state-changing operation. Run it only after the user explicitly requests a reset and the exact vault path has been verified:
...
Reset clears synchronization metadata; it does not replace a backup or remove existing Markdown files.
